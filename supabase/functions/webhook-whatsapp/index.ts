...
serve(async (req: Request) => {
  ...
  if (req.method === "GET") {
    ...
    console.log("WhatsApp verification request - mode:", mode, "token:", token);
    ...
    if (mode === "subscribe" && token) {
      ...
      if (integration?.webhook_verify_token === token) {
        console.log("WhatsApp webhook verified for business:", integration.business_id);
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Load business automation settings + limits.
      const { data: business } = await supabase
        .from("businesses")
        .select("chatbot_enabled, ai_reply_enabled, language_preference, greeting_message, flyer_cooldown_hours, office_hours, monthly_conversation_limit, monthly_ai_reply_limit")
        .eq("id", businessId)
        .single();
      ...
      for (const message of messages) {
        ...
        // Check usage limits before generating AI response
        ...
        if (chatbotEnabled && aiReplyEnabled && !limitReached) {
          ...
          if (aiResult?.reply) {
            ...
            await sendWhatsAppMessage(supabase, businessId, message.sender_phone_or_handle!, aiResult.reply);
