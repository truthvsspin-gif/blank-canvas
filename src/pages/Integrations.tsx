...
export default function Integrations() {
  ...
  async function handleSave() {
    ...
    if (!error) {
      setOriginalData({ ...formData, webhook_business_id: businessId || "" })
      await fetchWhatsAppHealth()
    }
  }

  const hasChanges = JSON.stringify(formData) !== JSON.stringify(originalData)
  const whatsappWebhookUrl = businessId
    ? `https://ybifjdlelpvgzmzvgwls.supabase.co/functions/v1/webhook-whatsapp?business_id=${encodeURIComponent(businessId)}`
