...
import { parseWhatsAppPayload } from "@/services/messageIngest"
...
export async function POST(request: Request) {
  ...
  let messages
  try {
    messages = parseWhatsAppPayload(payload)
  } catch (error) {
    const message = error instanceof Error ? error.message : "Invalid WhatsApp payload."
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to store WhatsApp messages."
  ...
  for (const message of messages) {
    try {
      ...
      const aiReply = await buildWhatsAppAiReply(message)
      const replyText = aiReply?.text || (await buildWhatsAppAutoReply(message))?.text
      if (replyText) {
        const outgoingResult = await sendOutgoingMessage(message, replyText)
